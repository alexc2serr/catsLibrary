...
function serveStaticFile(reqPath) {
  ...
  return serializeResponse({
    ...
    headers: { 'Content-Type': MIME_TYPES[ext] || 'application/octet-stream', 'Content-Length': buf.length },
...
function handleRequest(rawData, remoteAddress) {
  let parsedReq;
  try { parsedReq = parseRequest(rawData); }
  catch (err) {
    return serializeResponse({
  ...
  if (!auth.authenticated) {
    ...
    return serializeResponse({
      ...
      headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="usj-https"', ...CORS_HEADERS },
      body: JSON.stringify({ success: false, error: auth.reason }),
    });
  }

  const raw = authRouter(parsedReq) || catRouter(parsedReq) || ownerRouter(parsedReq) || serveStaticFile(parsedReq.path);
  if (raw) {
    const injected   = injectHeaders(raw, CORS_HEADERS);
    const statusMatch = injected.match(/HTTP\/1\.1 (\d+)/);
    logRequest(parsedReq, { statusCode: statusMatch ? +statusMatch[1] : 200 }, remoteAddress);
    return injected;
  }
  ...
  return serializeResponse({
    ...
    body: JSON.stringify({ error: `Cannot ${parsedReq.method} ${parsedReq.path}` }),
  });
}
...
function createTLSServer({ port = 3443, host = '127.0.0.1' } = {}) {
  ...
  const tlsServer = tls.createServer(tlsOptions, (socket) => {
    ...
    socket.on('error', (err) => {
      if (err.code !== 'ECONNRESET') console.error(`[TLS SOCKET ERROR] ${err.message}`);
    });
  });
  ...
  return {
    start() {
      tlsServer.listen(port, host, () => {
        console.log(`\x1b[32m[TLS SERVER]\x1b[0m Listening on \x1b[36mhttps://${host}:${port}\x1b[0m`);
        console.log(`\x1b[33m[INFO]\x1b[0m  Traffic is encrypted via TLS`);
      });
    },
    stop() { tlsServer.close(); },
    _server: tlsServer,
  };
}
